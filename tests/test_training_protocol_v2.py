...
def test_report_is_read_only_and_missing_assets_are_compact(tmp_path, monkeypatch):
    ...
    assert "Canonical JSON files remain the source of truth" in markdown
    assert "| 100 | — |" in markdown
    assert "| audit | not run |" in markdown
...
def test_report_write_only_creates_summary_md(tmp_path):
    ...
    assert path.is_file()
    assert "Do not edit this file manually" in path.read_text(encoding="utf-8")
