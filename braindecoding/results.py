...
_REPORT_NOTICE = (
    ...
    "> Canonical JSON files remain the source of truth.\n"
...
def _story_score(block):
    ...
    if story.get("available") is False:
        return f"— ({story.get('reason', 'not available')})"
...
def render_run_report(config):
    """只读取 canonical JSON，生成紧凑的人类可读 Markdown。"""
    ...
    lines = [
        ...
        f"- subjects: `{identity['subject_scope']}`",
        f"- seed: `{identity['seed']}`",
        f"- model condition: `{'neural_context' if config['model'].get('use_transformer') else 'word'}`",
        f"- checkpoint SHA-256: `{checkpoint.get('sha256') or '—'}`",
        "",
        "## Training",
        "",
        "| field | value |",
    ...
    if training is None:
        ...
        lines.extend(
            [
                f"| budget | {_display(budget.get('value'))} {_display(budget.get('type'))} |",
                f"| completed epochs | {_display(block.get('completed_epochs'))} |",
                f"| completed updates | {_display(block.get('completed_updates'))} |",
                f"| best epoch | {_display(selection.get('best_epoch'))} |",
                f"| best update | {_display(selection.get('best_update'))} |",
                f"| selection metric | {_display(selection.get('metric'))} |",
                f"| best validation value | {_display(selection.get('best_value'))} |",
                f"| stop reason | {_display(details.get('stop_reason'))} |",
                f"| final learning rate | {_display(details.get('final_learning_rate'))} |",
            ]
        )
        if config["training"].get("warm_start_from"):
            lines.extend(
                [
                    f"| warm start | {config['training']['warm_start_from']} |",
                    f"| frozen brain epochs | {_display(config['training'].get('freeze_brain_encoder_epochs'))} |",
                ]
            )
    ...
    lines.extend(
        [
            ...
            "## Validation",
            "",
            "| N | Support | Macro Top-1 | Macro Top-10 | Top-1 | Top-10 | Median rank | MRR | Story OVMI |",
            "|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
        ]
    )
    ...
    lines.extend(
        [
            ...
            "## Validation audit",
            "",
            "| Control | Macro Top-1 | Macro Top-10 | Median rank | MRR | Story OVMI |",
            "|---|---:|---:|---:|---:|---:|",
        ]
    )
    ...
    if not controls:
        ...
        for name in ("clean", "temporal_shift", "structure_only"):
            ...
            lines.append(
                f"| {name} | {_display(block.get('macro_recall_at_1'))} | "
        ...
        lines.append(
            f"| donor_swap mean | {aggregate_text('macro_top1')} | "
    ...
    lines.extend(
        [
            ...
            "## Provenance",
            "",
            f"- scientific config SHA-256: `{scientific_sha or '—'}`",
            f"- event table SHA-256: `{event_sha or '—'}`",
            f"- checkpoint SHA-256: `{checkpoint.get('sha256') or '—'}`",
            f"- N50 vocabulary manifest SHA-256: `{vocab_sha or '—'}`",
            f"- test status: `{(training or {}).get('test_status', 'locked_not_evaluated')}`",
            "",
        ]
    )
    return "\n".join(lines)
