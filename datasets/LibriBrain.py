...
# 兼容旧调用入口；公共文本工具的实现只保留在 braindecoding.data.text。
...
def build_event_table(config) -> pd.DataFrame:
    ...
    for record in _recording_files(config):
        ...
        words["source_event_row"] = words.index.astype(int)
    ...
    event_table = event_table.sort_values(
        ["session", "onset_seconds", "word_index", "source_event_row"]
    ).reset_index(drop=True)
    return add_event_contract(event_table)


def add_event_contract(event_table: pd.DataFrame) -> pd.DataFrame:
    """保持当前加载顺序，追加 LibriBrain100 中文公共字段。"""
    material_ids = pd.Series(
        [
            f"LibriBrain100|{task}|session-{int(session):02d}"
            for task, session in zip(event_table["task"], event_table["session"])
        ],
